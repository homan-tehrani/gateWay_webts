...
#
#
# async def parseUrl(request: Request):
...
#         if str(method).lower() != url['method']:
#             # Log unauthorized method attempt
#             thread = threading.Thread(target=saveLog, args=(request, 4470, body, "method"))
#             thread.start()
#             return False
#
#         # Check if 'path' key is present in the retrieved URL
#         if 'path' not in url or not url['path']:
#             thread = threading.Thread(target=saveLog, args=(request, 4471, body, "method"))
#             thread.start()
#             return False
#
#         try:
...
#         except Exception as e:
#             thread = threading.Thread(target=saveLog, args=(request, 4472, body, "cache.set"))
#             thread.start()
...
#     if path:
#         return path
#     thread = threading.Thread(target=saveLog, args=(request, 4473, request.body, "path dose not exist"))
#     thread.start()
#     return False
#
#
# #
# # except Exception as e:
# #     thread = threading.Thread(target=saveLog, args=(request, 4474, body, f"{e}"))
# #     thread.start()
# #     return JSONResponse(content="parseUrl", status_code=400)
#
#
# async def existUrl(request):
#     body = await request.body()
#     # Attempt to parse the URL using the parseUrl method
#     try:
#         path = await parseUrl(request)
#         # If a valid path is obtained, update path and return True
#         if not path:
#             return False
#         path = path
#         return True
...
#         thread = threading.Thread(target=saveLog, args=(request, 4475, body, f"{e}"))
#         thread.start()
#
...
#             url = f"{path['path']}?{request.query_params}"
#         else:
#             url = f"{path['path']}"
#
...
#             response = requests.request(method, url, headers=headers, data=await request.body())
#             if response is None:
...
#             return response
#     except Exception as e:
#         thread = threading.Thread(target=saveLog, args=(request, 4476, body, f"{e}"))
#         thread.start()
...
#         thread = threading.Thread(target=saveLog, args=(request, 4464, body, 'not existUrl'))
#         thread.start()
#
#         #  response for client
...
#             # Make a request to the external API without caching
#             print("--BUG NOT Cache URL ", url)
#             response = requests.request("get", url, headers=headers, data=await request.body())
#             if response is None:
#                 print("____NONE url ", url)
#                 print("____NONE body  ", await request.body())
#                 print("____NONE response  ", response)
...
#         thread = threading.Thread(target=saveLog, args=(request, 4465, body, f"{callService.text}"))
#         thread.start()
#         print("ERROR in resposne ", callService.text)
#         return JSONResponse(content=f"srvice was error ", status_code=400)
#     try:
#         callServiceContent = response.json()
#     except Exception as e:
#         callServiceContent = response.text
#     print(callServiceContent)
#     print('[[[[[[[[[[[[[[[[[[[callServiceContent]]]]]]]]]]]]]]]]]]]')
#     # if "id" in callServiceContent:
#     #     thread = threading.Thread(target=saveLog,
#     #                               args=(request, callServiceContent['id'], body, callServiceContent))
#     #     thread.start()
#     # else:
#     #     # set loge for dont exist log code
#     #     thread = threading.Thread(target=saveLog, args=(request, 4468, body, callServiceContent))
#     #     thread.start()
#
#     #  response for client
#     # return JSONResponse(content=callServiceContent, status_code=response.status_code)
#     return {'content': await request.body(), "status_code": response.status_code}
#     # except Exception as e:
#     #     thread = threading.Thread(target=saveLog, args=(request, 4469, body, f"{e}"))
#     #     thread.start()
